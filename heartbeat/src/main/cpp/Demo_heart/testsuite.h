...
static void
mavlink_test_sysinfo(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg) {
    ...
    memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_sysinfo_pack(system_id, component_id, &msg, packet1.sysState,
                             packet1.voltageBattery);
    mavlink_msg_sysinfo_decode(&msg, &packet2);
    MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

    memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_sysinfo_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg, packet1.sysState,
                                  packet1.voltageBattery);
    mavlink_msg_sysinfo_decode(&msg, &packet2);
    MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
...
static void
mavlink_test_Demo_heart(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg) {
