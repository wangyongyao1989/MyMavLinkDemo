 ...
 */
    ...
    namespace impl {
        ...
        template<typename _T>
        typename std::enable_if<std::is_floating_point<_T>::value, typename UintBuffer<_T>::Type>::type
        to_little_endian(_T data) {
        ...
        template<typename _T>
        typename std::enable_if<std::is_integral<_T>::value, typename UintBuffer<_T>::Type>::type
        to_little_endian(_T data) {
        ...
        typename std::enable_if<std::is_floating_point<_TOutput>::value, _TOutput>::type
        to_host_from_little_endian(_TInput data) {
            static_assert(sizeof(_TInput) == sizeof(_TOutput),
                          "Size of input and output must match");
        ...
        template<typename _TOutput, typename _TInput,
                class = typename std::enable_if<std::is_unsigned<_TInput>::value>::type>
        typename std::enable_if<std::is_integral<_TOutput>::value, _TOutput>::type
        to_host_from_little_endian(_TInput data) {
            static_assert(sizeof(_TInput) == sizeof(_TOutput),
                          "Size of input and output must match");
...
void mavlink::MsgMap::operator>>(_T &data) {
    ...
    if (static_cast<ssize_t>(sizeof(_T)) <= remaining_non_zero_data) { // field is not truncated
        ...
        size_t non_zero_count = std::max<decltype(remaining_non_zero_data)>(remaining_non_zero_data,
                                                                            0);
